...
async function toggleTheme() {
  ...
  try {
    ...
    tabs.forEach(tab => {
      chrome.tabs.sendMessage(tab.id, { type: 'theme-changed', theme }).catch(() => {});
    });
  } catch (error) {
...
async function loadSettings() {
  try {
    const settings = await chrome.storage.sync.get(['provider', 'apiKey', 'baseUrl', 'model']);
...
async function loadAdvancedSettings() {
  try {
    ...
    const defaultsResponse = await chrome.runtime.sendMessage({ type: 'get-default-prompts' });
...
document.getElementById('quick-menu-shortcut').addEventListener('keydown', async (e) => {
  ...
  if (e.key === 'Escape') {
    ...
    const input = document.getElementById('quick-menu-shortcut');
    const settings = await chrome.storage.sync.get('quickMenuShortcut');
    const shortcut = settings.quickMenuShortcut || { ctrl: true, shift: true, key: 'Space' };
  ...
  // Save immediately
  await chrome.storage.sync.set({ quickMenuShortcut: currentShortcut });
...
async function initializePopup() {
  ...
  try {
    const settings = await chrome.storage.sync.get('enablePopupWorkflow');
    ...
    if (popupWorkflowEnabled) {
      ...
      if (tabs.length > 0) {
        try {
          const response = await chrome.tabs.sendMessage(tabs[0].id, { type: 'get-selection' });
...
async function saveCollapsedState(header, isCollapsed) {
  try {
    const sectionId = header.textContent.trim();
    const settings = await chrome.storage.sync.get('collapsedSections');
    const collapsedSections = settings.collapsedSections || {};
    ...
    await chrome.storage.sync.set({ collapsedSections });
  } catch (error) {
