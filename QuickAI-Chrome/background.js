...
async function addToHistory(entry) {
  try {
    ...
    const { saveHistory } = await chrome.storage.sync.get('saveHistory');
...
async function createContextMenus() {
  try {
    ...
    const { showContextMenu } = await chrome.storage.sync.get('showContextMenu');
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  if (AI_COMMANDS.some(cmd => cmd.id === info.menuItemId)) {
    ...
    try {
      const response = await chrome.tabs.sendMessage(tab.id, { type: 'get-context' });
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      if (i < retries - 1) {
        ...
        try {
          await chrome.scripting.executeScript({ target: { tabId }, files: ['marked.min.js', 'katex.min.js', 'content.js'] });
          await chrome.scripting.insertCSS({ target: { tabId }, files: ['katex.min.css', 'content.css'] });
...
async function handleAICommand(commandId, selectedText, tabId, contextInfo = {}, customInstructions = null) {
  ...
  try {
    ...
    const settings = await chrome.storage.sync.get(['apiKey', 'baseUrl', 'model', 'provider']);
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'open-settings') {
    ...
    if (tabId) {
      ...
      chrome.tabs.sendMessage(tabId, { type: 'get-context' }).then(contextInfo => {
